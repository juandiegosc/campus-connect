...
- **Último cambio completado**: `db-seeder` (Fase 14) — `SeedDatabase(Action<IServiceProvider>)` en `MigrationExtensions` + `IdentityDbInitializer` que siembra 4 usuarios al primer arranque (director1/secretaria1/finanzas1/docente1, contraseña `Admin1234!`, idempotente). Encadenado tras `MigrateDatabase<IdentityDbContext>()` en Identity.API/Program.cs.
- **Cambio previo**: `db-consolidation` (Fase 13) + `messaging-endpoint-isolation` (ADR-076).
- **Seeder Identity**: usuarios de arranque listos — `director1/Admin1234!` (Direccion) permite hacer login sin bootstrap SQL manual.
- **Flujo dev local (dotnet run)**: infra en Docker (sin `--profile services`) + micros/Gateway con `dotnet run`. `UseUrls(8080)` y la selección de Ocelot (`ocelot.json` vs `ocelot.Local.json`) son condicionales a `DOTNET_RUNNING_IN_CONTAINER`. Puertos locales: Gateway 5287, Identity 5245, Academic 5157, Payments 5235, Attendance 5188, Notifications 5185, Analytics 5247. Auto-migración EF + seed al arrancar. Ver `docs/DEPLOYMENT.md`.
- **DB única (Fase 13)**: TODAS las bases viven en UNA instancia Postgres `cc-postgres` en `localhost:5438`. Bases: identity_db, academic_db, payments_db, attendance_db, notifications_db, analytics_db — creadas por `infra/postgres/init-databases.sql`. En Docker los servicios usan `Host=postgres;Port=5432`; en local `localhost:5438`. Sigue siendo base-por-servicio a nivel lógico.
- **Estado**: `backend-bootstrap` COMPLETO + `identity-service` COMPLETO + `academic-service` Phase 1-4 COMPLETO + `payments-service` Phase 1-3 COMPLETO + `attendance-service` Phase 1 COMPLETO. Pendientes: **Notifications, Analytics** (stubs sin dominio — últimos 2 bounded contexts).
...
- **Próximo cambio SDD**: `notifications-service-phase1` (cubre patrón Point-to-Point con SendNotificationCommand) → luego `analytics-service-phase1` (vista analítica / CQRS read model). Ambos cierran los patrones requeridos por la consigna.
